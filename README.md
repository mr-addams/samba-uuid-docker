...
- `GUEST_OK=yes` and `FORCE_USER=root` are set for convenience. For production use, create dedicated Samba users and disable guest access.
- NFS is exported with `no_root_squash` — NFS clients can access files as root. Restrict the export to specific IPs if needed (edit the `/etc/exports` generation in `entrypoint.sh`).
...
- `GUEST_OK=yes` и `FORCE_USER=root` настроены для удобства. В продакшне создайте отдельных пользователей Samba и отключите гостевой доступ.
- NFS экспортируется с `no_root_squash` — NFS-клиенты могут обращаться к файлам с правами root. При необходимости ограничьте экспорт конкретными IP (отредактируйте генерацию `/etc/exports` в `entrypoint.sh`).
